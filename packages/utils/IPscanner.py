...
def main():
    ...
    if args.test:
        ...
        print(f"[test] Detected local IP {local_ip}, testing on {cidr}")
    else:
        if args.cidr:
            cidr = args.cidr
        else:
            ...
            print(f"Auto-detected IP {local_ip}, defaulting to {cidr}")
    ...
    print(f"Scanning {len(ips)} addresses from {cidr} with {args.workers} workers, timeout={args.timeout}ms...")
    start = datetime.utcnow()
    alive = scan_ips(ips, workers=args.workers, timeout_ms=args.timeout)
    elapsed = (datetime.utcnow() - start).total_seconds()

    print(f"Scan finished in {elapsed:.2f}s. Hosts up: {len(alive)}")
    if alive:
        print("Live hosts:")
        for ip, rtt in alive:
            try:
                ...
                name = ""
            print(f"  {ip}\t{rtt:.0f}ms\t{name}")

        utils.debug(str(alive), "info")
